...
**Purpose:** Record anything that **diverges** from the original specs ([prd.md](prd.md), [schema.md](schema.md), [tech_stack.md](tech_stack.md), [backend.md](backend.md)), **follow-up plans** ([plan/](../plan/)), or **bug fixes** that change behavior or architecture. This keeps context when the codebase moves ahead of the written plan.
...
- **Original intent:** [phase17.md](../plan/phase17.md) — PDPA delete, Bruno coverage, Railway deploy notes.
...
- **Original intent:** [phase11.md](../plan/phase11.md) — match `MedicationSchedule.timeLocal` to Bangkok wall time.
...
- **Type:** bugfix-behavior
- **Original intent:** [phase14.md](../plan/phase14.md) — cursor pagination optional.
...
- **Original intent:** [phase17.md](../plan/phase17.md) suggests `DELETE /api/v1/me`; some teams use 204 No Content.
- **What we did instead:** **200** with `{ "data": { "deleted": true } }` for consistency with other `{ data }` responses.
- **Why:** Matches [backend.md](backend.md) success shape; easier for clients than empty 204.
...
- **What we did instead:** This file + links from [plan/README.md](../plan/README.md) and [backend.md](backend.md).
- **Why:** Preserve context when implementation, plans, and PRD diverge.

### 2026-04-12 — `GET /api/v1/medications` requires `familyMemberId`

- **Type:** bugfix-behavior
- **Original intent:** List endpoint shape implied by [phase10.md](../plan/phase10.md) (filter by member).
